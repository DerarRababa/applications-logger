...
const handleSearch = (employeeName,applicationType,actionType,fromDate,toDate,applicationID)=>{
  ...
  let data =  data.data.result.auditLog
    ...
    if(toDate){
      data = data.filter((obj)=>obj.creationTimestamp &&( new Date( new Date(obj.creationTimestamp.split(' ')[0]).toLocaledatatring()).valueOf() <= new Date(new Date(toDate).toLocaledatatring()).valueOf() ))
    }
    if(fromDate){
      data = data.filter((obj)=>obj.creationTimestamp &&( new Date( new Date(obj.creationTimestamp.split(' ')[0]).toLocaledatatring()).valueOf() >= new Date(new Date(fromDate).toLocaledatatring()).valueOf() ))
    }
 ...
 useEffect(() => { 
    ...
    if (data) {
      setApplicationsTypes([...new Set( data.data.result.auditLog.filter((obj)=> obj.applicationType != null).map((obj)=>obj.applicationType))])
      setActionTypes([...new Set( data.data.result.auditLog.filter((obj)=> obj.actionType != null).map((obj)=>obj.actionType))])
  ...
  return (
    <div className=" px-4">
     ...
     <FiltrationsSection handleSearch={handleSearch} applicationsTypes={applicationsTypes} actionsTypes={actionsTypes} />
      

      <div className="overflow-x-auto relative shadow-md sm:rounded-lg">
        <TabelSection mainData={mainData} pagination={pagination} />
       <PaginationSection setPagination={setPagination}  mainData={mainData} pagination={pagination} data={data} />
      </div>
    </div>
  );
};
